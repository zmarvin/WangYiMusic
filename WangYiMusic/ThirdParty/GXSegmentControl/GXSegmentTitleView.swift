...
//        let seleIndexPath = IndexPath(item: self.selectedIndex, section: 0)
        let specialIndexPath = IndexPath(item: self.config.specialIndex, section: 0)
        let seleCell = cell
        let specialCell = self.collectionView.cellForItem(at: specialIndexPath) as? GXSegmentTitleCell
        for subCell in self.collectionView.visibleCells {
            if subCell === seleCell && seleCell === specialCell{
               (subCell as? GXSegmentTitleCell)?.updateSpecialCellTitle(isChecked: false, animated: animated)
            }else if subCell === seleCell {
                (subCell as? GXSegmentTitleCell)?.updateCellTitle(isChecked: true, animated: animated)
            }else if subCell === specialCell{
                (subCell as? GXSegmentTitleCell)?.updateSpecialCellTitle(isChecked: false, animated: animated)
            }else {
...
extension GXSegmentTitleView: UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.isSpecial = (indexPath.item == self.config.specialIndex)
        return cell
    }
}
