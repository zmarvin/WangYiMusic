...
public class GXSegmentTitleCell: UICollectionViewCell {
    ...
    public var isSpecial: Bool = false {
        didSet {
            if isSpecial {
                self.titleLabel.textColor = self.config.specialIndexColor
            }
        }
    }
...
public extension GXSegmentTitleCell {
    ...
    func updateSpecialCellTitle(isChecked: Bool, animated: Bool) {
        self.updateCellTitle(isChecked: isChecked, animated: animated)
        self.titleLabel.textColor = self.config.specialIndexColor
    }
